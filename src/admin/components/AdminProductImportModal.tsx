            ...
            <>
              ...
              {file && !parsing && selection.grid.rows.length > 0 && (
                <ImportRowSelector
                  ...
                  note="Bỏ tích những dòng bạn KHÔNG muốn nhập. Lưu ý: nhiều dòng liên tiếp có thể thuộc cùng một sản phẩm (các biến thể) — bỏ tích dòng đầu (có tên sản phẩm) sẽ ảnh hưởng tới biến thể của nó."
                />
              )}
            </>
          )}
          ...
          {step === 'result' && result && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
                ...
                <StatCard icon={<Layers className="size-4" />} label="Biến thể" value={result.createdVariantCount} tone="muted" />
              </div>
